...
import { generarPDF } from "../utils/Descargar_PDF";
...
function useMenu() {
  ...
  const [mover_columna, setMover_columna] = useState(false);

  const [copiar_filas, setCopiar_filas] = useState(false);
  const [copiar_final, setCopiar_final] = useState(false);
  const [eliminar_filas, setEliminar_filas] = useState(false);
  ...
  const [mensaje_alerta, setMensaje_Alerta] = useState<string>("");
  ...
  const handleDescargar = () => {
    ...
    const blob = new Blob([JSON.stringify(filas, null, 2)], {
      type: "application/json",
    });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = "Plantilla.json";
    link.click();
  };
  ...
  const handleMover_Columna = () => {
    setMover_columna(!mover_columna);

    if (!mover_columna) {
      alert("Se ha activado la opción para ajustar la columna");
      const elementos = document.querySelectorAll(".resize-handle");
  ...
  const handleCopiar_Filas_Select = (id: number) => {
    if (copiar_final) {
      setFilas((prevFilas) => {
        const filaOriginal = prevFilas.find((fila) => fila.id === id);
        if (!filaOriginal) return prevFilas;
        ...
        // Agregar la duplicada al final
        return [...prevFilas, filaDuplicada];
      });
    } else {
      setFilas((prevFilas) => {
        const index = prevFilas.findIndex((fila) => fila.id === id);
        ...
        // Insertar la fila duplicada después del original
        return [
          ...prevFilas.slice(0, index + 1),
          filaDuplicada,
          ...prevFilas.slice(index + 1),
    ...
    handleAlerta("✅ La fila ha sido duplicada y agregada al final.");
  };

  const handleEliminar_filas = () => {
    setEliminar_filas(!eliminar_filas);

    if (!eliminar_filas) {
      alert("Seleccione las filas a eliminar");
    }
  };

  const handleEliminar_Filas_Select = (id: number) => {
    setFilas((prevFilas) => prevFilas.filter((fila) => fila.id !== id));

    handleAlerta("🗑️ La fila ha sido eliminada correctamente.");
  };
  ...
  const handleAlerta = (mensaje: string) => {
    setMensaje_Alerta(mensaje);
  ...
  return {
    ...
    copiar_filas,
    handleCopiar_filas,
    handleCopiar_Filas_Select,
    handleAlerta,
    eliminar_filas,
    handleEliminar_filas,
    handleEliminar_Filas_Select,
    mensaje_alerta,
    mover_columna,
    handleMover_Columna,
  };
